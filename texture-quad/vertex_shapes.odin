...
// END OF PENTAGONS
// QUAD
create_quad_texture_vertex_buffer_layout:: proc() -> wgpu.VertexBufferLayout {
	// va short for Vertex Attribute
	// va:[2]wgpu.VertexAttribute
	va[0] = wgpu.VertexAttribute {
		format = .Float32x3, // will be the datatype for position in Vertex Struct
	...
	va[1] = wgpu.VertexAttribute {
		format = .Float32x2, // will be the datatype for color in Vertex Struct
		// offset = size_of([3]f32), // the location in the Vertex Struct color data
		offset = 12, // the location in the Vertex Struct color data
		shaderLocation = 1,
	}
	...
	return wgpu.VertexBufferLayout {
		stepMode = .Vertex,
		arrayStride = size_of(TextureVertex),
		// arrayStride = size_of(Vertex),
		attributeCount = 2, // There's two elments in the vertex attribute array
		attributes = raw_data(va[:]), // Attributes is a multipointer
	}
...
create_quad :: proc() -> (wgpu.Buffer, wgpu.Buffer) {
	vertices:[4]TextureVertex = {
	    { position = {-0.5, 0.5, 0.0}, tex_coords = {0.0, 0.0}}, // A
	    { position = {0.5, 0.5, 0.0}, tex_coords = {1.0, 0.0}}, // B
	    { position = {0.5, -0.5, 0.0}, tex_coords = {1.0, 1.0}}, // C
	    { position = {-0.5, -0.5, 0.0}, tex_coords = {0.0, 1.0}}, // D
	}
