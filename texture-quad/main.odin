...
state: struct {
	...
	index_buffer: wgpu.Buffer, // STores the index buffer for the drawn shapes
	image: ^png.Image, // A pointer to the image loaded (not yet a wgpu texture)
	texture:wgpu.Texture, // The texture created from the image
...
main :: proc() {
	...
	state.image = load_image("./image.png")
	...
	// assignments
	state.texture = create_texture()
	
	state.texture_view = create_texture_view()
	state.sampler = create_sampler()

	texture_size:wgpu.Extent3D
	texel_copy_texture_info := create_texel_copy_texture_info()
	texel_copy_buffer_layout := create_texel_copy_buffer_layout()

	wgpu.QueueWriteTexture(
		queue = state.queue,
		destination= &texel_copy_texture_info,
		data = raw_data(state.image.pixels.buf[:]),
		dataSize = uint(len(state.image.pixels.buf) * 4), 
		dataLayout = &texel_copy_buffer_layout,
		writeSize = &state.texture_size,
	)

	state.bind_group_layout = create_bind_group_layout()
	state.bind_group = create_bind_group()
	
	// state.vertex_buffer,state.index_buffer = create_pentagon()
	state.vertex_buffer,state.index_buffer = create_quad()
	...
	vb_layout[0] = create_quad_texture_vertex_buffer_layout()
